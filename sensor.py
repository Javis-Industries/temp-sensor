...
humidity_gauge = Gauge(
    "humidity_pct", "Humidity Percentage", ["location", "sensor_type", "hostname"]
)
sensor_info = Info("sensor", "Sensor information")

# Sensor health metrics
sensor_read_errors = Gauge(
...
last_successful_read = Gauge(
    "sensor_last_successful_read_timestamp",
...
while True:
    try:
        ...
        temp_c_gauge.labels(location=SENSOR_LOCATION, sensor_type="DHT22", hostname=hostname).set(
            round(temp_c, 1)
        )

        temp_f_gauge.labels(location=SENSOR_LOCATION, sensor_type="DHT22", hostname=hostname).set(
            round(temp_f, 1)
        )

        humidity_gauge.labels(location=SENSOR_LOCATION, sensor_type="DHT22", hostname=hostname).set(
            round(humidity, 1)
        )

        last_successful_read.labels(location=SENSOR_LOCATION, hostname=hostname).set(time.time())
        ...
        error_count += 1
        sensor_read_errors.labels(location=SENSOR_LOCATION, hostname=hostname).set(error_count)
